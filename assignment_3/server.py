...
def image_processing():
    ...
    for model in ["COCO", "MPI"]:
        ...
        image_with_skeleton = hpe.image_with_skeleton(image, key_points, pose_pairs)
        imwrite(f"images/{current_time}_image_with_key_points_{model}_model.jpeg", image_with_key_points)
        imwrite(f"images/{current_time}_image_with_skeleton_{model}_model.jpeg", image_with_skeleton)
...
if __name__ == "__main__":
    ...
    if not (os.path.exists("pose/coco/pose_iter_440000.caffemodel") and os.path.exists("pose/mpi/pose_iter_160000.caffemodel")):
